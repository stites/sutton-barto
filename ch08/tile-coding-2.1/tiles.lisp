...
available at http://rlai.cs.ualberta.ca/RLAI/tiles.html.
...
(defun tiles (num-tilings memory-size-or-ctable floats &optional ints)
  ...
  (let* ((memory-size (ct-size ct))
    ...
    (cond ((equal ccheck (aref cdata j))
          ...
          (t (loop with h2 = (+ 1 (* 2 (hash-UNH ints num-ints MAX-LONGINT/4)))
                   ...
                   until (when (equal (aref cdata j) -1)
                           (setf (aref cdata j) ccheck)))))
    j))
...
(defun ct-usage (ct)
  (loop for d across (ct-data ct) count (not (equal d -1))))
...
(defun tiles-wrap (num-tilings memory-size-or-ctable floats wrap-widths &optional ints)
  "returns list of num-tilings tiles corresponding to variables (floats and ints), 
